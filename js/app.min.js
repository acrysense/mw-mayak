...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Находим все кнопки с классом nav-map-amenities__tab
  var tabs = document.querySelectorAll('.nav-map-amenities__tab');
  var dotButtons = document.querySelectorAll('.nav-map__dots');
  tabs.forEach(function (tab) {
    tab.addEventListener('click', function () {
      var selectedType = tab.getAttribute('data-type');
      var parentItem = tab.closest('.nav-map-amenities__item');

      // Если у родителя уже есть класс is--active, удаляем его и снимаем активность с dot-кнопок
      ...
      // Иначе добавляем/убираем класс is--active у элементов nav-map-amenities__item
      tabs.forEach(function (btn) {
        btn.closest('.nav-map-amenities__item').classList.remove('is--active');
      });
      parentItem.classList.add('is--active');

      // Добавляем/убираем класс is--active у элементов nav-map__dots по data-type
      dotButtons.forEach(function (dot) {
        if (dot.getAttribute('data-type') === selectedType) {
  ...
  // Добавляем событие для кнопок nav-map__dots
  dotButtons.forEach(function (dotButton) {
    dotButton.addEventListener('click', function () {
      var selectedType = dotButton.getAttribute('data-type');

      // Если текущая dot-кнопка уже активна, то снимаем активность и убираем класс у всех элементов
      if (dotButton.classList.contains('is--active')) {
        dotButton.classList.remove('is--active');
        tabs.forEach(function (tab) {
          tab.closest('.nav-map-amenities__item').classList.remove('is--active');
        });
        return;
      }

      // Добавляем/убираем класс is--active у всех nav-map__dots
      dotButtons.forEach(function (dot) {
        return dot.classList.remove('is--active');
      });
      dotButton.classList.add('is--active');

      // Добавляем/убираем класс is--active у элементов nav-map-amenities__item
      tabs.forEach(function (tab) {
        var parentItem = tab.closest('.nav-map-amenities__item');
