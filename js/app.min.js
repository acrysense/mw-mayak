...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var prevButton = document.querySelector('.gallery__slider .swiper-button-prev');
  var nextButton = document.querySelector('.gallery__slider .swiper-button-next');
  var cursorPrev = document.querySelector('.gallery__slider .swiper-button-prev .swiper-button-circle');
  var cursorNext = document.querySelector('.gallery__slider .swiper-button-next .swiper-button-circle');
  function moveCursor(event, cursorElement) {
    var x = event.clientX;
    var y = event.clientY;
    cursorElement.style.transform = "translate(".concat(x - 50, "px, ").concat(y - 50, "px)"); // Центрирование кружка
  }

  prevButton.addEventListener('mousemove', function (event) {
    return moveCursor(event, cursorPrev);
  });
  nextButton.addEventListener('mousemove', function (event) {
    return moveCursor(event, cursorNext);
  });
});